import { Component, inject, input } from '@angular/core';
import { RouterModule } from '@angular/router';

import SharedModule from 'app/shared/shared.module';
import { FormatMediumDatetimePipe } from 'app/shared/date';
import { DataUtils } from 'app/core/util/data-util.service';
import { IMensajeContacto } from '../mensaje-contacto.model';
import { NgClass } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { MensajeContactoService } from '../service/mensaje-contacto.service';

@Component({
  selector: 'jhi-mensaje-contacto-detail',
  templateUrl: './mensaje-contacto-detail.component.html',
  imports: [SharedModule, RouterModule, FormatMediumDatetimePipe, NgClass, FormsModule],
})
export class MensajeContactoDetailComponent {
  mensajeContacto = input<IMensajeContacto | null>(null);
  respuesta = '';

  protected mensajeContactoService = inject(MensajeContactoService);
  protected dataUtils = inject(DataUtils);

  byteSize(base64String: string): string {
    return this.dataUtils.byteSize(base64String);
  }

  openFile(base64String: string, contentType: string | null | undefined): void {
    this.dataUtils.openFile(base64String, contentType);
  }

  previousState(): void {
    window.history.back();
  }

  enviarRespuesta(): void {
    const mensaje = this.mensajeContacto();

    if (!mensaje?.id || !this.respuesta.trim()) {
      return;
    }

    this.mensajeContactoService.responder(mensaje.id, this.respuesta).subscribe(() => {
      this.previousState();
    });
  }
}
